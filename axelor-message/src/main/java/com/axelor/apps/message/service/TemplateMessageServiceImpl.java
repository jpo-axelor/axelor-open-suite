...
public class TemplateMessageServiceImpl implements TemplateMessageService {
  ...
  public Message generateMessage(Long objectId, String model, String tag, Template template)
    ...
    Templates templates;
    Map<String, Object> templatesContextMap = Maps.newHashMap();
    ...
    if (modelObj != null) {
      ...
      initMaker(objectId, model, tag, template.getIsJson(), templatesContextMap);
    ...
    if (!Strings.isNullOrEmpty(template.getContent())) {
      content = templates.fromText(template.getContent()).make(templatesContextMap).render();
    }

    if (!Strings.isNullOrEmpty(template.getAddressBlock())) {
      addressBlock =
          templates.fromText(template.getAddressBlock()).make(templatesContextMap).render();
    }

    if (!Strings.isNullOrEmpty(template.getSubject())) {
      subject = templates.fromText(template.getSubject()).make(templatesContextMap).render();
      log.debug("Subject ::: {}", subject);
    }

    if (!Strings.isNullOrEmpty(template.getFromAdress())) {
      from = templates.fromText(template.getFromAdress()).make(templatesContextMap).render();
      log.debug("From ::: {}", from);
    }

    if (!Strings.isNullOrEmpty(template.getReplyToRecipients())) {
      replyToRecipients =
          templates.fromText(template.getReplyToRecipients()).make(templatesContextMap).render();
      log.debug("Reply to ::: {}", replyToRecipients);
    }

    if (template.getToRecipients() != null) {
      toRecipients =
          templates.fromText(template.getToRecipients()).make(templatesContextMap).render();
      log.debug("To ::: {}", toRecipients);
    }

    if (template.getCcRecipients() != null) {
      ccRecipients =
          templates.fromText(template.getCcRecipients()).make(templatesContextMap).render();
      log.debug("CC ::: {}", ccRecipients);
    }

    if (template.getBccRecipients() != null) {
      bccRecipients =
          templates.fromText(template.getBccRecipients()).make(templatesContextMap).render();
    ...
    if (template.getSignature() != null) {
      signature = templates.fromText(template.getSignature()).make(templatesContextMap).render();
    ...
    messageService.attachMetaFiles(message, getMetaFiles(template, templates, templatesContextMap));
  ...
  public Set<MetaFile> getMetaFiles(
      Template template, Templates templates, Map<String, Object> templatesContextMap)
  ...
  public Map<String, Object> initMaker(
      ...
      String tag,
      boolean isJson,
      Map<String, Object> templatesContextMap)
      throws ClassNotFoundException {

    if (isJson) {
      templatesContextMap.put(tag, JPA.find(MetaJsonRecord.class, objectId));
    } else {
      Class<? extends Model> myClass = (Class<? extends Model>) Class.forName(model);
      templatesContextMap.put(tag, JPA.find(myClass, objectId));
    }

    return templatesContextMap;
  }
  ...
  protected Map<String, Object> computeTemplateContexts(
    ...
    for (TemplateContext templateContext : templateContextList) {
      ...
      templatesContextMap.put(templateContext.getName(), result);
    }
